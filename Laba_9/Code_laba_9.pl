...
pr9_11_start:-Alfavit=[a,b,c,d,e,f],write("Введите N = "),read(N),write("Введите K = "),read(K),write("Введите M = "),read(M),not(build_all(Alfavit,N, K,M)),tell('c:/Prolog/9_11.txt'),not(build_all(Alfavit,N, K,M)),told.
